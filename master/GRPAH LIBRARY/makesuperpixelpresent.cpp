//@Authors 
//:Nirwan Dogra , Kartik Singal , Aman Gupta
//#include "stdafx.h"
#include<queue>
...
vector<Mat> make_descriptors(vector< vector<Point> > superpixel,Mat img)
{
   ...
   for(int  k=0; k<superpixel.size(); k++)
   {
      ...
      // cout<<k<<" "<<endl;
      ret.push_back(bowdescriptor1);//push the histogram into the ret vector
   }
   //now below we compute the net histograms that is the summation
   for(int i=0; i<superpixel.size(); i++)
   {
      ...
      for(int j=0; j<superpixel.size(); j++)
      {
         ...
         if(GR[i][j]<=N && ret[j].rows!=0 && ret[i].rows!=0)
         {
            ...
            ret[i]=ret[i]+ret[j];//add to ret[i]th the histogram computed for the others
            cnt++;
         }
      }
      ret[i]=ret[i]/cnt;//take the average of the histograms
   }
   //just print the graphs for debugginh
   cout<<" GRAPH "<<endl;
   for(int i=0; i<40; i++)
   {
      for(int j=0; j<40; j++)
...
static vector<vector<Point> >  make_superpixels(Mat original)
{
   ...
   int nr_superpixels = atoi("2000");//2000
   int nc = atoi("60");//60
   double step = sqrt((w * h) / (double) nr_superpixels);
   Slic slic;
   // cvShowImage("fsdrwer",lab_image);
   //while(waitKey()!=27);
   slic.generate_superpixels(lab_image, step, nc);
   //cvShowImage("fsdrwe",lab_image);
   //while(waitKey()!=27);
   slic.create_connectivity(lab_image);

   //imshow(" superpixel image ",lab_image);
   CvScalar colo= CV_RGB(255,0,0);
   slic.display_contours(lab_image , colo );
   cvShowImage("fsd",lab_image);
   while(waitKey()!=27);
   Mat ret;
   ...
   cout<<endl;*/
   //between every two superpixels we try to 
   for(int i=0; i<slic.clusters.size(); i++)
   {
      for(int j=0; j<slic.clusters[i].size(); j++)
      {
   ...
   //after the graph has been constructed apply floyd_warshall
...
//      }
      ...
      for(int  k=0; k<superpixel.size(); k++)
      {
         ...
         detector.detect(newimg,keypoint1);
         bowde.compute(newimg,keypoint1,bowdescriptor1);//histograms computed here
         bowdescriptor1=desc[k];
         if(bowdescriptor1.rows==0)
         {
           //othheriwise code will crash coz bowdescriptor is null
            continue;
         }
         int clas=get_class(mask_image,x1,y1,x2,y2);//this will compute the class of this newimage found in 
         //that region of the original image
         training_data.push_back(bowdescriptor1);//push the bowdescriptor1 into training data
         training_data_back.push_back(bowdescriptor1);//push the histogram
         label.push_back((float)clas);//push the class
...
static void train_usingdata()
{
   ...
   bool res=svm.train(training_data,labels,Mat(),Mat(),params);//pass the training data and 
   //labels to the svm which then computes and makes the partition
...
static Mat test_image(Mat tempp)
{
...
int main()
{
   ...
   train_usingdata();
   int cnt=-1;
   int ar[5]={349,101,2,14};
   for(int i=349; i<=365; i++)
   {
      stringstream ss;
      ss << path_Bikes << "bike_"<<setfill('0') << setw(3) << (ar[++cnt]) <<".image.png";
      cout<<ss.str()<<endl;
      Mat send=imread(ss.str(),1);
      //imshow("fsdf",send);
      //while(waitKey()!=27);
      Mat newimg=test_image(send);
      stringstream ss2;
      ss2<< "C:\\Users\\NIRWAN\Desktop\\BIKES USING CRF\\bike\\" <<setfill('0') << setw(3) << ar[cnt] <<".png";
      imwrite(ss2.str(),newimg );
   }
   return 0;
}
